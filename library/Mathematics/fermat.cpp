...
/*
概要  ：フェルマーの小定理を用いた,組み合わせの余りを求める問題(nCr % mod)の高速化

引数  ：int n, int r

返り値：long long(nCr % mod)
...
int main() {
	...
	FermatCombination fc(n, mod);     // 初期化と nCi % mod の計算
	long long ans = fc.combine(n, r); // nCr % modの値の取得

	cout << n << " C " << r << " % mod = " << ans << endl;
